...
router.get("/api/deliverystate", (req, res) => {
  let queryShopsInfo =
    "SELECT it.*, es.nombre_estado, mp.nombre_imagen, SUM(cantidad) as cantidadTotal FROM INVENTARIO_TIENDAS it, estados es, MUESTRAS_PRODUCCION mp WHERE it.id_estado = 0 AND it.id_estado = es.id_estado AND it.referencia = mp.referencia GROUP BY numero_de_orden";
...
router.get("/api/getactualinventory", (req, res) => {
  let queryShopsInfo =
    "SELECT it.*, es.nombre_estado, mp.nombre_imagen, SUM(cantidad) as cantidadTotal FROM INVENTARIO_TIENDAS it, estados es, MUESTRAS_PRODUCCION mp WHERE it.id_estado = 1 AND it.id_estado = es.id_estado AND it.referencia = mp.referencia GROUP BY numero_de_orden";
